...
namespace Gravitation.Screens.Menu
{
    class SelectShipScreen: BaseScreen, IDrawableScreen
    {
        ...
        void settingsWindow_okClicked(object sender, EventArgs e)
        {
            ...
            if (ships.Count() == 1)
                constructoArgs = new object[] { (object)new DataClasses.GameConfiguration("../../../Maps/firstLevel.xml", ships[0].Ship, null) };
            else
                constructoArgs = new object[] { (object)new DataClasses.GameConfiguration("../../../Maps/firstLevel.xml", ships[0].Ship, ships[1].Ship) };
